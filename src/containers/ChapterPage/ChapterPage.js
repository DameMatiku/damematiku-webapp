...
class ChapterPage extends Component {
  ...
  renderChapter = () => {
    ...
    return ( 
      <div>
        {!!chapter && chapter.videos.length > 0 && chapter.videos.map((video, key) => <VideoView key={key} video={video} />)}
        {(!chapter || chapter.videos.length === 0) && <EmptyChapterView chapter={chapter} />}
      </div>
    );
  };
